...
const SignupComponent = () => {
  ...
  const validateNickname = (nickname) =>
    /^[a-zA-Z0-9가-힣]{2,11}$/.test(nickname); // 한글, 영문, 숫자만, 2~11자
  const validateAccountNumber = (accountNumber) =>
    /^[0-9]{10,15}$/.test(accountNumber); // 숫자만, 10~15자리
  ...
  const onSubmitHandler = (event) => {
    ...
    const isPasswordValidLength =
      memberPassword.length >= 6 && memberPassword.length <= 20;
    ...
    const isAccountNumberFormatValid =
      validateAccountNumber(memberAccountNumber);
      ...
      <Form className=" bg-white rounded" onSubmit={onSubmitHandler}>
        ...
        <Form.Group className="mb-2  " controlId="memberEmail">
          <Row>
            ...
            {useEmail && <p style={{ color: 'red' }}>{emailMessage}</p>}
            {!useEmail && <p style={{ color: 'blue' }}>{emailMessage}</p>}
          </Row>
        </Form.Group>
        ...
        <Form.Group className="mb-2 mb-4 " controlId="memberPassword">
          ...
          {isValidLength && (
            <p className="passwordHelpBlock" style={{ fontSize: '12px' }} muted>
              비밀번호는 6 ~ 20자여야 합니다.
            </p>
          )}
          {!isValidLength && (
            <p
              className="passwordHelpBlock"
        ...
        <Row className="mb-2 ">
          <Col className="col-6">
            <Form.Group controlId="memberName">
              ...
              />
              {isNameValid && (
                <p style={{ fontSize: '12px' }}>한글 2~11자로 입력해 주세요.</p>
              )}
              {!isNameValid && (
                <p style={{ color: 'red', fontSize: '12px' }}>
                  이름은 한글 2~11자로 입력해 주세요.
                </p>
              )}
            </Form.Group>
          </Col>
            ...
            <Form.Group controlId="memberNickname">
              ...
              {isNicknameValid && (
                <p style={{ fontSize: '12px' }}>
                  한글, 영문, 숫자 포함 2~11자로 입력해 주세요.
                </p>
              )}
              {!isNicknameValid && (
                <p style={{ color: 'red', fontSize: '12px' }}>
                  닉네임은 한글, 영문, 숫자 포함 2~11자로 입력해 주세요.
                </p>
              )}
            </Form.Group>
          </Col>
